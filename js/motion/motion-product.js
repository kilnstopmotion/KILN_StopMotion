(() => {
  ...
  const copy = {
    vi: {
      ...
      motionGuideIntro:'Lập kế hoạch quỹ đạo và spacing giữa các frame trước khi animate. Cuộn chuột ngay trong vùng demo để cảm nhận cách chuyển động đi từ F01 tới frame cuối.',
      ...
      motionGuideControlCopy:'Chọn một kiểu easing, sau đó cuộn chuột trên vùng Motion Guide để xem spacing và tốc độ thay đổi trực tiếp.',
      motionGuideScrollHint:'Cuộn chuột để di chuyển · F01 → F08',motionGuideCurrentFrame:'Frame hiện tại',motionGuideSpeed:'Tốc độ chuyển động',motionGuideProgress:'Tiến trình',
    ...
    en: {
      ...
      motionGuideIntro:'Plan a path and the spacing between frames before animating. Scroll inside the demo to feel the movement progress from F01 to the final frame.',
      ...
      motionGuideControlCopy:'Choose an easing pattern, then scroll over the Motion Guide to see spacing and speed change directly.',
      motionGuideScrollHint:'Scroll to move · F01 → F08',motionGuideCurrentFrame:'Current frame',motionGuideSpeed:'Motion speed',motionGuideProgress:'Progress',
  ...
  function initMotionGuide() {
    ...
    // Keep the original HTML as a no-JS fallback, then replace only its simple illustration.
    ...
    demo.innerHTML = `
      <div class="motion-guide-head">
        ...
        <div class="motion-guide-rules" aria-label="Spacing explanation">
          <div class="motion-guide-rule">
            <span class="motion-guide-rule-visual" aria-hidden="true"><i></i><i></i><i></i><i></i></span>
            <span><strong data-guide-i18n="motionGuideSlowRule">${text('motionGuideSlowRule')}</strong><small data-guide-i18n="motionGuideSlowRuleCopy">${text('motionGuideSlowRuleCopy')}</small></span>
          </div>
          <div class="motion-guide-rule">
            <span class="motion-guide-rule-visual is-far" aria-hidden="true"><i></i><i></i><i></i></span>
            <span><strong data-guide-i18n="motionGuideFastRule">${text('motionGuideFastRule')}</strong><small data-guide-i18n="motionGuideFastRuleCopy">${text('motionGuideFastRuleCopy')}</small></span>
          </div>
        </div>
      </div>

      <div class="motion-guide-workspace" data-guide-wheel-zone tabindex="0" aria-label="Interactive Motion Guide">
        <div class="motion-guide-stage">
          ...
          <svg class="motion-guide-svg" viewBox="0 0 900 460" role="img" aria-label="Eight frame markers distributed along a motion path">
            ...
            <g data-guide-ghosts aria-hidden="true">
              <circle class="motion-guide-ghost" r="11" opacity=".22"></circle>
              <circle class="motion-guide-ghost" r="10" opacity=".14"></circle>
              <circle class="motion-guide-ghost" r="9" opacity=".08"></circle>
            </g>
        ...
        <aside class="motion-guide-controls">
          ...
          <div class="motion-guide-speed">
            <div class="motion-guide-speed-head"><span data-guide-i18n="motionGuideSpeed">${text('motionGuideSpeed')}</span><output data-guide-speed-label>${text('motionGuideSlow')}</output></div>
            <div class="motion-guide-speed-track"><div class="motion-guide-speed-fill" data-guide-speed-fill></div></div>
          </div>

          <div class="motion-guide-progress">
            <div class="motion-guide-control-label"><span data-guide-i18n="motionGuideProgress">${text('motionGuideProgress')}</span><output data-guide-progress-output>0%</output></div>
            <div class="motion-guide-progress-track" data-guide-progress-track></div>
            <div class="motion-guide-progress-labels"><span>F01</span><span>F08</span></div>
          </div>
        </aside>
      </div>`;
    ...
    const capabilityCopy = document.querySelectorAll('[data-i18n="toolGuideCopy"]');
    capabilityCopy.forEach(node => { node.textContent = text('toolGuideCopy'); });
    ...
    const recipeDots = name => Array.from({length:6},(_,i) => {
      const x = easing[name](i / 5) * 100;
      return `<i style="--x:${x.toFixed(2)}%"></i>`;
    }).join('');

    recipeRoot.innerHTML = recipeDefs.map(([name,labelKey,metaKey]) => `
      <button type="button" class="motion-guide-recipe${name === currentEase ? ' is-active' : ''}" data-guide-ease="${name}" aria-pressed="${name === currentEase}">
        <strong data-guide-i18n="${labelKey}">${text(labelKey)}</strong>
        <small data-guide-i18n="${metaKey}">${text(metaKey)}</small>
        <span class="motion-guide-mini" aria-hidden="true">${recipeDots(name)}</span>
      </button>`).join('');

    function pointForRawProgress(raw) {
      const clamped = Math.max(0, Math.min(1, raw));
      const eased = easing[currentEase](clamped);
      return path.getPointAtLength(eased * pathLength);
    }
    ...
    function renderMarkers() {
      ...
      for (let i = 0; i < FRAME_COUNT; i += 1) {
        ...
        const point = pointForRawProgress(t);
    ...
    function derivative(name, t) {
      ...
      const epsilon = .002;
      const a = fn(Math.max(0, t - epsilon));
      const b = fn(Math.min(1, t + epsilon));
      return (b - a) / Math.max(epsilon, Math.min(1, t + epsilon) - Math.max(0, t - epsilon));
    }
    ...
    function updateProgress(next, fromSlider = false) {
      ...
      ghosts.forEach((ghost, index) => {
        const ghostProgress = Math.max(0, progress - ((index + 1) * .035));
        const ghostPoint = pointForRawProgress(ghostProgress);
    ...
    recipeRoot.addEventListener('click', event => {
      ...
      if (!button) return;
      setEase(button.dataset.guideEase);
    });
    ...
    document.querySelectorAll('[data-lang]').forEach(button => {
      button.addEventListener('click', () => requestAnimationFrame(refreshLocalizedText));
    });
  }
  ...
  function init() {
    ...
    if (guideDemo && desktop) {
      gsap.fromTo(guideDemo,
        { y: 26, opacity: .82, rotationX: 4 },
        { y: 0, opacity: 1, rotationX: 0, ease: 'none', scrollTrigger: { trigger: guideDemo, start: 'top 90%', end: 'top 52%', scrub: true } }
      );
    }
  }
